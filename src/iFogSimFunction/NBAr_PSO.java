...
public class NBAr_PSO {

	private static final int RUN = 30;
	private static double c1 = 1.5; // c1,c2大小在[0,4]之间，且c1+c2>4
	...
	private static final double X = 0.4;
	private static int[][] x = new int[pop_num][R];
	private static int[][] v = new int[pop_num][R];

	private static int[][] pbest = new int[pop_num][R]; 
	private static int[] gbest = new int[R]; 
	private static double[] pbest_fit = new double[pop_num];     // 个体最佳适应值
	private static double gbest_fit;                             // 种群最佳适应值
    ...
    static int[] case_num_RP = new int[RUN];
    static int[] case_num_NBA = new int[RUN];
    static int[] case_num_noNBA = new int[RUN];
	...
	public static void main(String[] args){
		...
		for(int run=0;run<RUN;run++)
		{
			...
			//初始化pbest,gbest,pbest_fit,gbest_fit
			...
			double[][] cos = new double[pop_num][pop_num];
			int path;
			int obj = 0;
...
//			int gbest_index = 0;
			...
			// 更新pbest,gbest,pbest_fit,gbest_fit
			for (int i = 0; i < pop_num; i++) 
			{
				if (fitness_x[i] > pbest_fit[i])
				{
					pbest_fit[i] = fitness_x[i];
					for(int j=0;j<R;j++)
						pbest[i][j] = x[i][j];
				}
			}
			for (int i = 0; i < pop_num; i++) 
			{
				if (fitness_x[i] > gbest_fit)
				{
					gbest_fit = fitness_x[i];
					for(int j=0;j<R;j++)
						gbest[j] = x[i][j];
				}
			}
			...
			{	
				for (int i = 0; i < pop_num; i++) {
					for (int j = 0; j < R; j++) {
						...
						v[i][j] = (int) (X * v[i][j] + c1 * r1 * (pbest[i][j] - x[i][j]) + c2 * r2 * (gbest[j] - x[i][j]));
//						v[i][j] = (int) (X * v[i][j] + c1 * (pbest[i][j] - x[i][j]) + c2 * (gbest[j] - x[i][j]));
						x[i][j] = v[i][j] + x[i][j];
//
						//x越界的反弹处理
						if (x[i][j] > ub[j]) {		
				...
				for (int i = 0; i < pop_num; i++) {
					...
					if (fitness_v[i] > fitness_x[i]) {
						for (int j = 0; j < R; j++)
							x[i][j] = v[i][j];                       //选择x和v中的更佳个体，并更新适应值
				...
				for (int i = 0; i < pop_num; i++)                     //gbest更新
				{
					pbest_time++;
					if (fitness_x[i] > gbest_fit)
					{
							...
							{//如果没有维度影响，此时只能随机
								...
								while(step>=1){//搜索个体数目大于step_length情况
									...
									for(int k=0;k<step_length+1;k++){
										...
										case_num_RP[run] = case_num_RP[run] +1;
									}
...
///检查该节点是否有分支存档		
								...
								if(PATH[target_path].charAt(node)!= 32 && NBArchives[node][j][PATH[target_path].charAt(node) - '0'] != Integer.MIN_VALUE) {
									...
									case_num[run] = case_num[run] + 1;           //评估次数更新
									case_num_NBA[run] = case_num_NBA[run] + 1;
									...
									while(step>=1){//搜索个体数目大于step_length情况
										...
										for(int k=0;k<step_length+1;k++){
											...
											fitness_temp[k] = benchmarkfunction(x[i], fun_num, target_path);     //评估个体
											case_num[run] = case_num[run] + 1;           //评估次数更新
											case_num_noNBA[run] = case_num_noNBA[run] + 1;
										}
										int best_index = getBestIndex(fitness_temp);
										x[i][j] = temp[best_index];
										best = temp[best_index];
			...
			System.out.println("case_num_RP["+run+"] = " + case_num_RP[run] ); 	
			System.out.println("case_num_NBA["+run+"] = " + case_num_NBA[run] ); 	
			System.out.println("case_num_noNBA["+run+"] = " + case_num_noNBA[run] ); 
